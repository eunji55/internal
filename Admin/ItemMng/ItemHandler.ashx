...
public class ItemHandler : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        try
        {
            ...
            if (objReqParam.strMethod.Equals("ItemList"))
            {
                intRetVal = getItemList(objReqParam, out objResParam, out strErrMsg);

                return;
            }
            else if (objReqParam.strMethod.Equals("ItemDel"))
            {
                intRetVal = itemDel(objReqParam, out strErrMsg);
    ...
    /// </summary>
    ///----------------------------------------------------------------------
    private int getItemList(ReqParam objReqParam, out ResParam objResParam, out string strErrMsg)
    {
        int    intRetVal     = 0;
        ...
        try
        {
            ...
            //프로시져 호출
            objDas.SetQuery("dbo.UP_ITEM_AR_LST"); //Execute Query 
    ...
    /// </summary>
    ///----------------------------------------------------------------------
    private int itemDel(ReqParam objReqParam, out string strErrMsg)
    {
        ...
        try
        {
            ...
            objDas.AddParam("@pi_intItemNo",            DBType.adInteger,       objReqParam.intItemNo,              0,          ParameterDirection.Input);
            ...
            //프로시져 호출
            objDas.SetQuery("dbo.UP_ITEM_TX_DEL"); //Execute Query 
